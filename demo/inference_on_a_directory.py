...
if __name__ == "__main__":
    ...
    parser.add_argument("--json_path", "-i", type=str,
    ...
    # loop until there is a file system event that the json file is created or modified
    while True:
        # create name based on current time in format: YYYY-MM-DD_HH-MM-SS
        job_title = time.strftime("%Y-%m-%d_%H-%M-%S")
        try:

            # check if the json file is created or modified
            already_checked_time = last_change_time is not None
            ...
            try:
                # load json object from json_path
